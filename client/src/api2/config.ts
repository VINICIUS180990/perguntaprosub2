...
const getApiKey = (): string => {
  ...
  if (!apiKey) {
    console.error('[CONFIG] ❌ VITE_GEMINI_API_KEY não encontrada!');
    console.error('[CONFIG] ❌ Certifique-se de que o arquivo .env.local existe com a chave da API');
    throw new Error('API Key do Gemini não configurada. Verifique o arquivo .env.local');
  }
...
export const AI_CONFIG = {
  ...
  get API_URL() { return `https://generativelanguage.googleapis.com/v1/models/${this.MODEL}:generateContent?key=${this.API_KEY}`; },
...
// === VALIDAÇÃO === //
try {
  AI_CONFIG.API_KEY; // Trigger validation
  console.log('[CONFIG] ✅ API Key carregada com sucesso');
} catch (error) {
  console.warn('[CONFIG] ⚠️ Erro ao carregar API Key:', error);
}
